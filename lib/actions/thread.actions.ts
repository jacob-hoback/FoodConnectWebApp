...
interface Params {
  ...
  path: string,
  likes: [], // just added for likes feature
  image: string,
}

export async function createThread({ text, author, communityId, path, image}: Omit<Params, 'likes'>) {
  try {
    connectToDB();
...
export async function likePost(threadId: string, likesArray: string[]) {
  
  try{
    const thread = await Thread.findById(threadId);
    if(!thread) throw Error;
    
    const updatedPost = await thread.save();
